...
public class RandomUtil {
	...
	public static void main(String[] args) {
		...
		for(int i = 0 ;i<10;i++){
			list.add(randomNumberFixLength(6));
		}
		...
		System.out.println("===================");
		while (true){
			finalCode = randomNumberFixLength(6);
			if(list.contains(finalCode)){
				continue;
			}else{
				break;
			}
		...
		System.out.println(getUniqueKey(6));
	}
     ...
     * @return
     */
    public static synchronized  String getUniqueKey(int randomNumberLength){
        String number = randomNumberFixLength(randomNumberLength);
